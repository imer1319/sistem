<template>
	<div class="modal fade" id="preguntasCreate" tabindex="-1" role="dialog" aria-labelledby="preguntasCreateLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<form @submit.prevent="storePregunta">
					<div class="modal-header">
						<h5 class="modal-title" id="preguntasCreateLabel">Registrar nueva pregunta</h5>
...
export default{
	...
	methods:{
		storePregunta(){
			...
			axios.post('/api/preguntas',formData)
			.then(res=>{
				$('#preguntasCreate').modal('hide');
				EventBus.$emit('add-pregunta', res.data);
				this.errors = []
				this.pregunta = {};
			})
			.catch(err => {
